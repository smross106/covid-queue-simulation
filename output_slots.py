...
size = 8
actors,hood = helper.setup2(64*size,5*(size**2),50)
    ...
    if tick%10==0:
        ...
        for i in actors:
            if type(i)==Actor:
                ...
                i.run(hour,hood,[randomSlotPref])
...
plt.title("100% 'random_slot_at_preference' technique, "+str(int(100*fed/len(actors)))+"% of people fed")
plt.xlim([0,24])
plt.ylim(0)
